...
def config_gpu(use_gpu=True):
    ...
    np.set_printoptions(suppress=True)  # 打印浮点数时不使用科学计数法
    os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'  # 设置环境变量，屏蔽TF的一些警告信息？
    try:
        if use_gpu:
            gpus = tf.config.experimental.list_physical_devices('GPU')  # 获得所有GPU的列表
            for gpu in gpus:
                tf.config.experimental.set_memory_growth(gpu, True)  # GPU内存自然增长，根据需要动态分配内存
        else:
            os.environ['CUDA_VISIBLE_DEVICES'] = '-1'  # 设置环境变量，表示不使用GPU
    except:
        pass


def get_gpu_temprature():
    output = os.popen("nvidia-smi -q | grep 'GPU Current Temp' | cut -d' ' -f 24").read()  # 乱码的原因，但是我不需要知道gpu温度
